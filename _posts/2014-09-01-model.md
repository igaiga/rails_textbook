...
既存のDBテーブルにカラムを増やすにはどうすれば良いでしょうか？前に作ったmigrationファイルを変更してもうまくいきません。各migrationファイルは1回だけ実行される仕組みなので、過去に実行されたmigrationファイルを変更しても、そのファイルは実行されないからです。そこで、新しいカラムを追加するには、新しいmigrationファイルを作ります。
