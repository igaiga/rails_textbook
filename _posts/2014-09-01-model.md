...
答えは `ApplicationRecord` クラスを継承しているからです。 `ApplicationRecord` クラスがモデルの仕事に必要な機能を持っています。それを継承しているBookクラスも同じ機能を持ちます。[^1]

[^1]: Rails 4.2の場合、`ApplicationRecord`の替わりに`ActiveRecord::Base`が使われていますが、考え方は同じです。
...
books appを作る一連のコマンドを入力した際に、以下のコマンドでDBを作成していました[^2]。
...
[^2]: Rails4.2以前では `rails db:migrate` の替わりに `bin/rake db:migrate` となります。
