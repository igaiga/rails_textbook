...
Railsには "rails cosole" という、Rubyでいうirbのような、1行ずつコードを実行する機能があります。前準備でつくったアプリへ移動して、rails consoleを使ってみましょう。ターミナルで`rails c`と実行してみてください（メッセージ中"in process 53813"の数字は実行するたびに異なります）。cはconsoleの頭文字です。
