...
たとえ話で説明すると、Gemfileはgemをインストールするための指示書です。bundle installコマンドを実行すると、指示書に従ってgemがインストールされます。Gemfile.lockは納品書といったところでしょうか。指示書に基づいて実際にインストールされたgemのバージョン情報などが書かれています。
