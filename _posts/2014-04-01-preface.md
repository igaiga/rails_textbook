...
本書の内容を実行する場合にはRubyおよびRailsの開発環境を作る必要があります。
...
Windowsの場合はWindows Subsystem for Linux(WSL)とrbenvを利用することで環境を構築することができます。

プログラムを入力する道具であるエディタは、好みのものがあればそれを利用してください。特にない場合は [Visual Studio Code](https://code.visualstudio.com/) をお勧めします。Visual Studio Codeは無料で利用することができます（ライセンスはMITライセンスです）。

#### WSLのインストール

WSLを使うとWindows上で利用できるLinux環境をインストールすることができます。インストールしたLinux上で、rbenvやaptなどLinux/UNIX向けツールを利用することでRailsの開発環境を構築できます。WSLを利用するためにはWindows10環境が必要になります。
...
つづいてrbenvをインストールします。以下のコマンドを実行してください。
...
Rubyをインストールします。Bashウィンドウで以下のコマンドを実行してください。
