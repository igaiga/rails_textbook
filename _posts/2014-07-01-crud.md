...
次に、以下のコマンドを実行します（メッセージ中 "20191012005727"、"0.xxxxs" などの数字は実行するごとに異なります）。
...
表示は `<% render book %>` で行っています。実は、表示部分は別のファイルに書いてあり、 `<%= render book %>` で埋め込まれるようになっています。

![renderの説明](assets/crud/index_view_form_screenshot.png)

TODO: パーシャル位置を示すスクショ加工画像 加工元assets/crud/books_index_data_2.png
...
2回目の実行では「Ruby超入門」のBookモデルオブジェクトが入って同様の処理が実行され、HTMLがつくられます。パーシャルビュー `_book.html.erb` をそれぞれのデータで合計2回埋め込んだHTMLがつくられ、それらを埋め込んだ `index.html.erb` からHTMLがつくられます。
