...
newアクションは @<tt>{@book = Book.new} の1行です。Book.newでBookクラスのインスタンスを作り、@bookインスタンス変数へ代入し、ビューへ渡します。Book.newでつくったBookクラスのインスタンスはタイトルとメモを格納できるようになっていますが、タイトルとメモのデータは空っぽです。
...
Railsコードの部分をもう少し詳しく見てみましょう。@<tt>{form.label :title} で "Title"という文字列を表示しています。その名の通り、ラベルの部分です。@<tt>{form.text_field :title} はその下にあるテキスト入力欄です。@<tt>{form} はformブロック内の変数で、ここではbookに関するformを記述するために使っています。見慣れない書き方かもしれませんが、ここはそう書くものだと思ってもらえれば大丈夫です。
...
メモの部分も同様です。@<tt>{form.label :memo} が "Memo" を表示する部分です。@<tt>{form.text_area :memo} がその下のテキスト入力欄を作ります。@<tt>{text_area} は先ほどの @<tt>{text_field} よりも広くて改行を入力できるテキスト入力欄を作るメソッドです。
...
//image[devtools_create_3][Chromeでリクエストを観察する(リクエストの中身)]{
//}




//image[devtools_create_4][Chromeでリクエストを観察する(リクエストの中身 form)]{
//}
...
実行結果を見ると、確かに @<tt>{params} の中にブラウザにて入力した値がHashの形で入っていることが分かりました。
...
たとえば、titleだけを更新したいケースがあり、titleだけを更新するformをつくったとします。@<tt>{Book.new(book_params)}でnewメソッドは、引数で受け取った値を自分のカラムへ代入します。このとき、book_paramsにtitleの情報だけがやってくればいいのですが、攻撃者はmemoの情報もパラメータとして飛ばすこともあります。StrongParametersで受け取り可能なパラメータを絞っていないと、プログラマの意図しないカラムが更新されてしまうことになります。
