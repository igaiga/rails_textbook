...
もしもPictureUploaderが見つからない旨のエラー（"Unable to autoload constant PictureUploader"など）が発生した場合は、rails serverを一度止めてからもう一度起動して、再アクセスしてみてください。また、 @<tt>{app/uploaders/picture_uploader.rb} ファイルが存在するかも確認してみてください。存在しない場合は @<tt>{rails g uploader Picture} コマンドが実行されていないケースが考えられます。
