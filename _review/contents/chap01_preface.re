...
また、プログラミングが初めての方で、本書を読んで「プログラムの基礎をもっと学んでから挑戦したい」「プログラムを読んで理解はできるが、自分で考えて書くのが難しい」と感じた方は、筆者の別著書である「ゼロからわかる Ruby超入門」（書籍情報はあとがきの章にあります）をお勧めします。



初めてプログラミングをする方へ向けた、Rubyプログラミングの入門書です。プログラミングの基礎である条件分岐、繰り返しの概念を図解を入れて丁寧に説明します。また、Railsで使う知識を広くカバーするように書いています。
...
ブラウザはつくったWebアプリへアクセスするための道具としてつかいます。お好きなものをご利用いただけますが、文中で説明につかわれている @<href>{https://www.google.co.jp/chrome/browser/,Chrome} 使うと分かりやすいです。



もしもここに書いている内容でうまくいかない場合は、 @<href>{https://railsgirls.jp/install,RailsGirlsガイド インストール・レシピ} のページが新しい情報で更新されているので、こちらも参考にしてみてください。
...
@<href>{https://brew.sh/,Homebrew} をインストールします。HomebrewはMacでさまざまな便利なプログラムをインストール、管理するためのツールです。
...
Windowsの場合はWSL（Windows Subsystem for Linux）とrbenvを利用することで開発環境をつくることができます。インストールしたWSL上でLinux向けツールを利用して開発します。WSLを利用するためにはWindows10以降が必要です。もしもWindows10よりも前のバージョンを利用している場合や、WSLのインストールがうまく行かないときには、 @<href>{https://railsgirls.jp/install,RailsGirlsガイド インストール・レシピ} のページにある「Windows 用セットアップ（WSLが使えない方向け）」を参考にしてみてください。



WSLをインストールする方法は @<href>{https://docs.microsoft.com/ja-jp/windows/wsl/install-win10,Windows Subsystem for Linux Installation Guide for Windows 10 のページ} に書かれていますので、こちらの手順でインストールしてください。また、 @<href>{https://railsgirls.jp/install,RailsGirlsガイド インストール・レシピ} のページにも同様の手順がスクリーンショットをつかって丁寧に説明されています。インストールするLinuxを選ぶところでは好きなものを選んでもらって構いませんが、特になければUbuntuを選択することをお勧めします。
...
ターミナルで以下のコマンドを実行してください。なお、最初のコマンドを実行する際にパスワードの入力を求められますが、先程ターミナルに入力したパスワードを入力してください。Windowsのパスワードではないことに注意してください。 @<tt>{sudo dpkg-reconfigure tzdata} を実行するとタイムゾーンを設定する画面が出ますので、「Asia」を矢印キーで移動して選択して、Tabキーを押してOkボタンに移動して、Enterキーを押してください。つづいて都市を選択する以下の画面が起動するので、「Tokyo」を矢印キーで移動して選択して、Tabキーを押してOkボタンに移動して、Enterキーを押してください。
...
Railsは最新バージョンのものをつかって問題ありませんが、もしも問題が出て進められなくなったときには、@<tt>{gem i rails -v 6.0.2.1}のようにバージョンを指定して、本書の内容と揃えることもできます。そのときには、以降のページで出てくる@<tt>{rails new アプリ名}コマンドを実行するときに、つかうバージョンを指定し@<tt>{rails _6.0.2.1_ new アプリ名}とします。



また、本書執筆時の最新バージョンであるRuby2.7.0とRails6.0.2.1の組み合わせは、ターミナルで多くのWarning（警告）メッセージが出ます。次のバージョンへ向けての互換性に関する警告が多く、有用で大切なものではありますが、ほかの重要なメッセージが見づらくなる問題もあります。RubyとRailsのバージョンをみなさんが実行するときの最新バージョンにすることで、Warningメッセージが少なくなっている可能性があります。できるだけ新しいバージョンを使うことをお勧めします。
