...
mkdirはフォルダを作成するコマンド、cdはターミナル内で現在のフォルダを移動するコマンドです。Windowsで普段使いなれている「コンピューター（エクスプローラー）」を起動するには、ターミナルで@<tt>{explorer .}と打つと現在のフォルダを開くことができます。MacでFinderを開くにはターミナルから@<tt>{open .}と打ちます。mkdirコマンドで作成したフォルダが存在することが確認できます。フォルダはターミナルからとエクスプローラー（MacではFinder）からとどちらから作成しても違いはないので、どちらでも問題ありません。
...
次は以下のコマンドを実行してみてください。@<tt>{rails s}コマンドはwebサーバを起動するコマンドで、sはserverの略です。
...
ここで実行したコマンド @<tt>{rails s} のsはserverの略です。省略したsでも、省略せずにserverでも、同じように動作します。
...
rails newコマンドがエラーで中断されたときには、表示されるエラーメッセージを手がかりに問題を解決する必要がありますが、これらの知識も役に立つかもしれません。また、エラーで中断したあとにリトライしたいときには、これらの処理が途中のどこかで止まっていることがあるので、アプリのフォルダを削除してからもう一度実行してみてください。
...
もしもrails gコマンドを打ち間違えて違うファイルをつくってしまったときは、打ち間違えたコマンドのgの部分をdにして再実行すると、rails gコマンドで作成したファイルをまとめて削除してくれます。たとえば、@<tt>{rails g controller hell index} とhelloをhellと打ち間違えた場合は、@<tt>{rails d controller hell index} コマンドを実行することで間違えてつくったファイル群を削除することができます（ターミナルでカーソルキーの↑キーを押すと、さきほど入力した内容が出てくるので、それを利用してgをdに直すと楽に実行できます）。
