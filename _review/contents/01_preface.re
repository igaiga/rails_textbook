...
@<href>{https://brew.sh,Homebrew} をインストールします。HomebrewはmacOSでさまざまな便利なプログラムをインストール、管理するためのツールです。以下のコマンド中の@<tt>{-fsSL}の後ろに表示上は改行をいれていますが、実行するときは次の行とつなげて1行のコマンドとして実行してください。
...
Homebrewを使ってrbenvをインストールします。rbenvはRubyをインストールして管理するための道具です。なおmacOS Catalinaからデフォルトのshellがbashからzshに変更されています。 @<tt>{echo $SHELL} を実行した結果が @<tt>{/bin/zsh} のようにzshであれば以下の通りですが、結果が @<tt>{/bin/bash} のようにbashであるときは、以下の 🌠.zshrcを.zprofileに置換 @<tt>{.zshrc} を @<tt>{.bash_profile} に置き換えて実行してください。
 ...
 * @<href>{https://code.visualstudio.com,Visual Studio Code 公式ページ}
 * @<href>{https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers,Visual Studio Code マーケットプレイス Dev Containers}
 * @<href>{https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ja,Visual Studio Code マーケットプレイス Japanese Language Pack for Visual Studio Code}



Dockerはコンテナと呼ばれる仮想環境パッケージをつかってアプリ実行環境（ここではRails開発環境）をつくることができるプラットフォームです。Docker Desktopなどをつかってインストールします。

 * Docker Desktop @<href>{https://docs.docker.com/desktop,https://docs.docker.com/desktop}
 ...
 * Rails new リリースページ @<href>{https://github.com/rails/rails-new/releases,https://github.com/rails/rails-new/releases}
...
ターミナルからrails-newコマンドをつかってRailsアプリをつくります。@<tt>{--devcontainer} オプションをつけることで、あわせてDev Container環境をつくります。次のコマンドではrails-newコマンドがRailsアプリexample@<b>{appをつくり、アプリ名と同名のexample}appフォルダをつくります。
...
次に、VS Codeを起動してDev Container環境をつかってみます。VS Codeの「ファイル」メニューから「開く」を選び、さきほどrails-newコマンドでつくったexample_appフォルダを選びます。
