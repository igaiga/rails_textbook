...
Railsには "rails console" という、1行ずつ入力したコードを実行する機能があります。Rubyが持っているirbをつかって、1行ずつ入力したRailsのコードを実行することができます。つくったアプリがあるフォルダへ移動して、rails consoleを使ってみましょう。ターミナルで@<tt>{rails c}と実行してみてください。cはconsoleの頭文字です。メッセージ中"in process 53813"の数字は実行するたびに異なります。
...
@<tt>{db/migrate/20191008234834_create_books..rb}
...
1行目 @<tt>{ActiveRecord::Migration[6.0]} の末尾にある数字はRailsのバージョンを表します。Rails6.0.xではこのように6.0となります。
...
長いので、着目するところだけ残して短くしましょう。まず、@<tt>{respond_to do |format|}はリクエストされたレスポンスの形式によって分岐させる文です。ブラウザでnew画面に内容を入力してリクエストを飛ばした場合は@<tt>{format.html}が選ばれます。もう一方のjsonはデータ形式の1つで、たとえばスマートフォンアプリなど、ブラウザ以外で表示させるときによく利用されます。formatに関する処理を除くと次のようになります。
...
@<tt>{@book.save}は成功するとtrue、失敗するとfalseを返します。 @<tt>{@book.save}の前に書いてあるifは分岐させる命令です。ifのあとに書かれた処理（ここでは@<tt>{@book.save}）がtrueのときはその後ろからelseの前までを、falseのときはelse以降endまでを実行します。



@<tt>{@book.save}に成功するとtrueを返すので、@<tt>{redirect_to @book, notice: 'Book was successfully created.'}が実行されます。redirect_toはリダイレクト（新たにリクエストを発行して画面遷移させる）指示で、ここではshowアクションへのリクエストが発生します。保存した本の詳細ページ（BooksControllerのshowアクション）を表示します。後ろのnotice部は画面に表示させる文を設定しています。
...
@<tt>{@book.save}に失敗するとfalseを返すので、ifのelse節、@<tt>{render :new}が実行されます。renderはコントローラの次の処理であるビューを指定します。ここでは@<tt>{app/views/books/new.html.erb}がビューとして使われ、新規入力画面、つまり直前で入力していたページが表示されます。



ところで、saveメソッドが失敗するのはどんなときでしょうか？DBへの接続エラーなどのアクシデントで失敗することもありますが、モデルにある検証機能「バリデーション」を使って、想定外の入力に対して保存を失敗させることもあります。たとえば、数字を期待している郵便番号入力欄に、数字以外の文字が入力されるようなケースは、バリデーションを実装することで入力画面へ戻しユーザーに再入力を促すことができます。バリデーションについてはここでは説明しませんが、便利な機能なので @<href>{https://railsguides.jp/active_record_validations.html,Rails ガイド: Active Record バリデーション} などの説明を参考にしてみてください。
...
ちなみに、Railsのgenerate機能は開発をアシストする機能なので、使わないで0から手でコードを書いても同様に動きます。つまり、rails gを使ってファイルを自動生成してから変更しても、0から手でコードを書いても、どちらも同じ結果になります。ほかのrails gコマンド、scaffoldやcontrollerなども全て同様です。
...
@<tt>{db/migrate/20191217005831_add_author_to_books.rb}
